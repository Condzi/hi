...
set cc_analyzers=    clangsa clang-tidy

if not exist %cc_dir% mkdir %cc_dir%
if not exist %cc_report_dir% set new=1

:: --- Run the analysis -------------------------------------------------------
if "%new%"=="1" %cc_analyze% -c -i skipfile.txt --output %cc_report_dir% %cc_compile_cmds% --analyzers %cc_analyzers% --analyzer-config 'clang-tidy:take-config-from-directory=true' --analyzer-config clangsa:AllEnable=true
